...
@section('content')
    <style>
        #publication-lang-tabs-edit .publication-lang-tab-btn {
            border-bottom: 2px solid transparent !important;
            border-radius: 0;
        }

        #publication-lang-tabs-edit .publication-lang-tab-btn.active {
            border-bottom-color: #f4a259 !important;
        }
    </style>

    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-2">
        <h3 class="mb-0">Nashr — tahrirlash</h3>
    ...
    <div class="card shadow-sm border-0 mb-4">
        <div class="card-body">
            <form action="{{ route('admin.publications.update', $publication) }}" method="POST" enctype="multipart/form-data">
                ...
                <ul class="nav nav-underline mb-3" id="publication-lang-tabs-edit">
                ...
                <div class="row g-3 publication-lang-panel-edit" data-lang-panel="uz">
                    <div class="col-md-12">
                        <label class="form-label">Sarlavha (UZ) *</label>
                        <input type="text" name="title_uz" value="{{ old('title_uz', $publication->title_uz) }}"
                            class="form-control @error('title_uz') is-invalid @enderror">
                        @error('title_uz') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                </div>

                <div class="row g-3 publication-lang-panel-edit d-none" data-lang-panel="ru">
                    <div class="col-md-12">
                        <label class="form-label">Sarlavha (RU) *</label>
                        <input type="text" name="title_ru" value="{{ old('title_ru', $publication->title_ru) }}"
                            class="form-control @error('title_ru') is-invalid @enderror">
                        @error('title_ru') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                </div>

                <div class="row g-3 publication-lang-panel-edit d-none" data-lang-panel="en">
                    <div class="col-md-12">
                        <label class="form-label">Sarlavha (EN) *</label>
                        <input type="text" name="title_en" value="{{ old('title_en', $publication->title_en) }}"
                ...
                <div class="row g-3 mt-1">
                    <div class="col-md-6">
                        ...
                        <select name="type" class="form-select @error('type') is-invalid @enderror">
                            @foreach (\App\Enums\PublicationType::cases() as $type)
                                <option value="{{ $type->value }}" @selected(old('type', $publication->type->value) === $type->value)>
                                    {{ $type->label() }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" min="0" name="order" value="{{ old('order', $publication->order) }}"
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="is_active" class="form-select @error('is_active') is-invalid @enderror">
                            <option value="1" @selected(old('is_active', $publication->is_active ? '1' : '0') == '1')>Aktiv</option>
                            <option value="0" @selected(old('is_active', $publication->is_active ? '1' : '0') == '0')>Nofaol</option>
                        </select>
                ...
                <div class="mt-4 d-flex gap-2 justify-content-end">
                    ...
                    <a href="{{ route('admin.publications.show', $publication) }}" class="btn btn-outline-info">Ko'rish</a>
    ...
    <x-admin.summernote-lang-tabs-script tabs-selector="#publication-lang-tabs-edit [data-lang]"
        panels-selector=".publication-lang-panel-edit" />
@endsection
